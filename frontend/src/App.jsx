...
function DashboardView({
  ...
  useEffect(() => {
    localStorage.setItem("jc_active_platform", activeTab);
  }, [activeTab]);

  const activePlatform = PLATFORM_CONFIG.find((p) => p.key === activeTab) || PLATFORM_CONFIG[0];
  const activeItems = grouped[activePlatform.key] || [];
    ...
    <>
      ...
      <PlatformTabs
        ...
        counts={Object.fromEntries(
          PLATFORM_CONFIG.map((p) => [p.key, (grouped[p.key] || []).length]),
        )}
      />
      ...
      {PLATFORM_CONFIG.map((p) => {
        ...
        return (
          <button
            ...
            className={`rounded-t-xl border-b-2 px-5 py-2 text-sm font-medium transition ${
              isActive
                ? "border-sky-300 bg-slate-900/70 text-white"
                : "border-transparent text-slate-200/60 hover:text-white"
            }`}
          >
            {p.label.replace(/ Sales$| Free$/, "")}{" "}
            <span
              className={`ml-1 rounded-full px-2 py-0.5 text-xs ${
                isActive
                  ? "bg-sky-400/20 text-sky-100"
                  : "bg-slate-800/60 text-slate-200/50"
              }`}
            >
              {count}
            </span>
    ...
    >
      ...
      >
        ...
        {rows === null && !errored ? (
          ...
          <div className="mt-6 text-sm text-slate-200/70">
            Price history not available for this deal yet. Check back after
            a few daily crawls.
          </div>
        ) : (
          <div className="mt-6 space-y-4">
            {min ? (
              <div className="rounded-2xl border border-emerald-300/40 bg-emerald-400/10 px-4 py-3 text-sm">
                ...
                {(() => {
                  const formatted = formatHistoryDate(min.recorded_at, "date");
                  return formatted !== "—" ? (
                    <>
                      {" "}
                      <span className="text-slate-200/60">on {formatted}</span>
                    </>
                  ) : null;
                })()}
              </div>
                  ...
                  {rows.map((row, i) => (
                    ...
                    >
                      <td className="px-4 py-2 text-slate-200/80">
                        {formatHistoryDate(row.recorded_at)}
                      </td>
